...
"""
function calcϕ(ψdisc::Array{Float64}, λdisc::Array{Float64}, F, P, r::Float64)
	# Initialize ϕ
	...
	for i = 1:length(ψdisc)
		for j = 1:length(λdisc)
			...
			n = choose_surface_normal(vc, F, P)
...
"""
function choose_surface_normal(vc::Array{Float64,2}, F, P)
	#flip vector to get outward vector from center
	r = -vc/norm(vc)

	#finding closest distance
	dist = zeros(1,size(F)[1])
	for face = 1:size(F)[1]
		a = F[face,1]
		b = F[face,2]
		c = F[face,3]
		...
		z0 = P[face,3]
		d = -(a*x0+b*y0+c*z0)
		dist[face] = -d/(a*r[1]+b*r[2]+c*r[3])
		if dist[face] < 0
			dist[face] = -dist[face] #make positive
			dist[face] = dist[face]+2
		end
	end
	(closest_face_dist,closest_face_ind) = findmin(dist)
	n = F[closest_face_ind[2],:]
	return n
end
"""
function get_areas
